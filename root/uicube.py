...
class CubeWindow(ui.ScriptWindow):
	...
	def LoadWindow(self):
		...
		self.openChestButtonMultiple.SetEvent(ui.__mem_func__(self.__OnAcceptButtonClick))
		self.openAmount = 1
	...
	def SetOpenAmount(self, amount):
		...
		self.openChestButtonMultiple.SetText(localeInfo.CRAFT_CUBE_NEW % self.openAmount)
		if self.needMoney:
			if self.gold >1:
				self.needMoney.SetText(localeInfo.NumberToMoneyString(self.openAmount*self.gold))
	...
	def Open(self):
		...
		self.openAmount = 1
		self.SetOpenAmount(1)	
		self.needMoney.SetText(localeInfo.NumberToMoneyString(0))	
	...
	def UpdateInfo(self, gold, itemVnum, count):
		self.gold = gold
		if self.needMoney:
			self.needMoney.SetText(localeInfo.NumberToMoneyString(self.openAmount*long(gold)))
	...
	def AgreeCrafting(self):
		self.CubeAccept = uiCommon.QuestionDialogCraft()
		self.CubeAccept.Open()	
	
		## Question Text
		questionText = 	localeInfo.CRAFT_ACCEPT_TEXT

		self.CubeAccept.SetText(questionText)				
		self.CubeAccept.SetAcceptEvent(lambda arg=TRUE: self.AgreedCrafting(arg))
		self.CubeAccept.SetCancelEvent(lambda arg=FALSE: self.AgreedCrafting(arg))
		self.CubeAccept = CubeAccept

		constInfo.SET_ITEM_QUESTION_DIALOG_STATUS(1)

	def AgreedCrafting(self, answer):
		if not self.CubeAccept:
			return
		if answer:
			for i in xrange(self.openAmount):	
				net.SendChatPacket("/cube make")

		self.CubeAccept.Close()
		self.CubeAccept = None
