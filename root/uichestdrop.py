...
class ChestDropWindow(ui.ScriptWindow):
	...
	def SetOpenAmount(self, amount):
		...
		self.openChestButtonSingle.SetText(localeInfo.CHEST_INSIGHT_OPENX_CHEST % self.openAmount)
		self.openChestButtonMultiple.SetText(localeInfo.CHEST_INSIGHT_OPENX_CHEST % self.openAmount)
	...
	def RefreshItems(self, chestVnum):
		...
		if not self.chestDrop.has_key(self.currentChest):
			chat.AppendChat(chat.CHAT_TYPE_INFO, "ERROR - Please inform a GameMaster.")
			self.Close()
			return
