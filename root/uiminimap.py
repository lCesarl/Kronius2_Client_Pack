...
class MiniMap(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		if app.ENABLE_PINGTIME:
			...
			nFPS = app.GetRenderFPS()

			self.time.SetFontColor(0.9, 0.7, 0.0)
			self.fps.SetFontColor(0.9, 0.7, 0.0)
			self.ping.SetFontColor(0.9, 0.7, 0.0)

			self.time.SetFontName(localeInfo.UI_DEF_FONT_LARGE)
			self.fps.SetFontName(localeInfo.UI_DEF_FONT_LARGE)
			self.ping.SetFontName(localeInfo.UI_DEF_FONT_LARGE)

			self.time.SetFeather()
			self.time.SetOutline()
			self.fps.SetFeather()
			self.fps.SetOutline()
			self.ping.SetFeather()
			self.ping.SetOutline()

			self.time.SetText("[%s]" % (currTime))
			self.fps.SetText("FPS: %s, " % (nFPS))
			self.ping.SetText("PING: %s" % (nPing))
