...
import uiInventory
import item
if app.ENABLE_RENDER_LOGIN_EFFECTS:
	import effect
...
class SelectCharacterWindow(ui.Window):
	...
	def Open(self):
		import chat
		chat.AppendChat(8, uiScriptLocale.LOCALE_UISCRIPT_PATH)
	...
	def MakeCharacter(self, index, id, name, race, form, hair, acce, weapon):
		...
		if app.ENABLE_SHININGS_ON_LOGIN:
			chr.ApplyShinings(index)
	...
	def SelectSlot(self, index):
		...
		if 0 != id:
			...
			if statesSummary > 0.0:
				self.destGauge =	[
										float(valueHTH) / maxStates,
										float(valueINT) / maxStates,
										float(valueSTR) / maxStates,
										float(valueDEX) / maxStates
									]
