...
class RefineDialogNew(ui.ScriptWindow):
	...
	def __LoadScript(self):
		...
		self.checkBox = ui.CheckBox_old()
		self.checkBox.SetParent(self)
		self.checkBox.SetPosition(0, 80)
		...
		self.checkBox.SetTextInfo(uiScriptLocale.AUTOREFINE)
		self.checkBox.Show()
		self.isLoaded = TRUE
	...
	def Open(self, targetItemPos, nextGradeItemVnum, cost, prob, type):
		...
		if not constInfo.IS_AUTO_REFINE:
			self.SetTop()
		self.Show()
	...
	def AppendMaterial(self, vnum, count):
		slot = self.__MakeSlot()
		slot.SetParent(self)
		slot.SetPosition(15, self.dialogHeight - 5)
		...
		thinBoard.SetPosition(50, self.dialogHeight - 5)
		...
		inventoryCount = player.GetItemCountByVnum(vnum)
		refinedItemVnum = player.GetItemIndex(self.targetItemPos)
		if refinedItemVnum == vnum:
			inventoryCount -= player.GetItemCount(self.targetItemPos)
			
		if inventoryCount < count:
			textLine.SetPackedFontColor(0xffFF0033)
		else:
			textLine.SetPackedFontColor(0xff40EF37)
		textLine.SetText("|cFFdddddd|H|h%s x%d|h|r (%d)" % (item.GetItemName(), count, inventoryCount))
	...
	def UpdateDialog(self):
		...
		newWidth = self.toolTip.GetWidth() + 60
		newHeight = self.dialogHeight + 80 + 10
		...
		if constInfo.IS_AUTO_REFINE == True:
			if constInfo.AUTO_REFINE_TYPE == 1:
				if constInfo.AUTO_REFINE_DATA["ITEM"][0] != -1 and constInfo.AUTO_REFINE_DATA["ITEM"][1] != -1:
					...
					if scrollIndex == 0:
						chat.AppendChat(8, str(scrollIndex) + " " + str((itemIndex % 10)) + " " + str(isFail))
						self.Close()
					else:
						chat.AppendChat(8, str(scrollIndex) + " " + str((itemIndex % 10)) + " " + str(isFail))
				...
				if npcData[0] != 0 and npcData[1] != -1 and npcData[2] != -1 and npcData[3] != 0:
					...
					if (isFail):
						chat.AppendChat(8, str((itemIndex % 10)) + " " + str(isFail))
						self.Close()
					else:
