...
class NewOfflineShopBoard(ui.ScriptWindow):
	...
	def __IsSaleableSlot(self, win , pos):
		...
		try:
			if app.__ENABLE_SPECIAL_INVENTORY_SYSTEM__:
				if not win in (player.INVENTORY, player.DRAGON_SOUL_INVENTORY, player.UPGRADE_INVENTORY, player.SKILL_BOOK_INVENTORY, player.STONE_INVENTORY):
					return False
	...
	def __OnClickMyShopOfferAcceptButton(self, offer):
		...
		offerPrice = str(offer.GetInfo()['price'])

		if (offerPrice.find("000")):
			offerPriceInK = offerPrice.replace("000", "k")
			if (offerPriceInK[-1] == "0"):
				offerPriceInK = offerPriceInK[0:2] + offerPriceInK[-1] + offerPriceInK[2:-1]
		else:
			offerPrice = str(localeInfo.MoneyFormat(offer.GetInfo()['price']))

		question 	= (localeInfo.OFFLINESHOP_ACCEPT_OFFER_QUESTION % (str(offerPriceInK)))
