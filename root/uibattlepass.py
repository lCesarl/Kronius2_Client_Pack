...
class ListBoxMissions(ui.Window):
	class Item(ui.Window):
		def __init__(self, missionId):
			...
			bgImage = ui.MakeExpandedImageBox2(self, ROOT_PATH + "mission_bg_normal.tga", 0, 0, "not_pick")
			iconImage = ui.MakeExpandedImageBox2(bgImage, iconName, 1, 2, "not_pick")
			bgGauge = ui.MakeExpandedImageBox2(bgImage, ROOT_PATH + "mission_progress_empty.tga", 47, 28, "not_pick")
			bgGaugeFull = ui.MakeExpandedImageBox2(bgGauge, ROOT_PATH + "mission_progress_full.tga", 8, 2, "not_pick")
			...
			for i in xrange(MISSION_REWARD_COUNT):
				rewardImage = ui.MakeExpandedImageBox2(self, "d:/ymir work/ui/game/belt_inventory/slot_disabled.tga", 187 + (32 * i), 6)
			...
			self.missionName = ui.MakeTextLineNew(bgImage, 53, 6, "")
...
class BattlePassWindow(ui.ScriptWindow):
	...
	def LoadMissionList(self):
		try:
			lines = pack_open("%s/battle_pass.txt" % app.GetLocalePath() , "r").readlines()
		except:
			import exception
	...
	def LoadWindow(self):
		...
		#	self.rankingButton = self.GetChild("RankingButton")
			...
			import exception
			exception.Abort("BattlePassWindow.LoadDialog.BindObject")
	...
	def SetMissionInfo(self, missionId):
		...
		if info_2 >= info_3:
			self.missionInfoDict[0].SetText(localeInfo.BP_STATUS + " " + localeInfo.BP_COMPLETE)
		else:
			self.missionInfoDict[0].SetText(localeInfo.BP_STATUS + " " + localeInfo.BP_PROGRESS)
		...
		self.missionInfoDict[3].SetText(localeInfo.BP_PERCENT + " %.1f%%" % float(info_2 * 100 / info_3))
	...
	def RefreshGeneralInfo(self):
		if self.missionList:
			if self.generalInfoDict.has_key(0) and BATTLE_PASS_NAMES.has_key(self.battlePassID):
				self.generalInfoDict[0].SetText("Name: %s" % str(BATTLE_PASS_NAMES[self.battlePassID]))
			...
			if self.generalInfoDict.has_key(1):
				self.generalInfoDict[1].SetText(localeInfo.BP_QUEST_COUNT % missionCount)
	
			if self.generalInfoDict.has_key(2):
				self.generalInfoDict[2].SetText(localeInfo.BP_COMPLETED_QUESTS % completedMissionCount)
	...
	def CreateRankingItems(self):
		for i in xrange(RANKING_MAX_NUM):
			...
			self.rankingTexts[i] = {
				"pos" : ui.MakeTextLine2(itemImage, False, False, 21, 9),
				"name" : ui.MakeTextLine2(itemImage, False, False, 110, 9),
				"time" : ui.MakeTextLine2(itemImage, False, False, 240, 9)
			}
			
		if not self.loadingBackground:
			self.loadingBackground = ui.ImageBox2()
		...
		if self.currentState == "GENERAL":
			if self.generalInfoDict.has_key(3):
				if self.battlePassEndTime > 0:
					...
					self.generalInfoDict[3].SetText(localeInfo.BP_REMAINING_TIME  % str(leftTime))
...
class BattlePassButton(ui.Window):
	def __init__(self):
		...
		self.SetPosition(wndMgr.GetScreenWidth() - 110, 225)
		self.SetSize(85, 23)
